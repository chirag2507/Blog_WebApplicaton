document.addEventListener('DOMContentLoaded', () => {
  ...
  (function setupFeed() {
    ...
    function parseLikes(text) {
      ...
      if (/k$/i.test(text)) return Math.round(parseFloat(text.replace(/k/i, '')) * 1000);
      if (/m$/i.test(text)) return Math.round(parseFloat(text.replace(/m/i, '')) * 1000000);
    ...
    function attachLikeHandlers() {
      ...
      feed.querySelectorAll('.like-btn').forEach(btn => {
        ...
        btn.addEventListener('click', function (e) {
          ...
          const current = parseLikes(countSpan ? countSpan.textContent : '0') + 1;
    ...
    function searchBlogs() {
      ...
      const filtered = currentList.filter(blog =>
  ...
  (function setupAnimations() {
    function run() {
      try {
        ...
        const ScrollTrigger = (gsapLib && gsapLib.ScrollTrigger) || window.ScrollTrigger;
        if (ScrollTrigger && gsapLib.registerPlugin) {
          try { gsapLib.registerPlugin(ScrollTrigger); } catch (e) {}
        }
        ...
        if (!document.querySelector('.ud-page')) {
          ...
          document.querySelectorAll('.reveal-clip').forEach(r => r.classList.add('show'));
          return;
        }

        // Nav entrance
        gsapLib.from('nav', { y: -20, opacity: 0, duration: 0.6, ease: 'power2.out' });
        ...
        tl.from('.eyebrow', { y: 8, opacity: 0, duration: 0.45 })
          ...
          .from('.hero-ctas .btn', { y: 12, opacity: 0, stagger: 0.12, duration: 0.45 }, '-=0.6')
          .from('.visual-card', { scale: 0.98, opacity: 0, duration: 1 }, '-=0.9');

        // animated gradient orb subtle rotation
        gsapLib.to('.gradient-orb', { rotation: 30, duration: 12, repeat: -1, ease: 'none' });
        ...
        if (ScrollTrigger) {
          gsapLib.to('.ud-hero-visual img', {
            yPercent: -8,
            ease: 'none',
            scrollTrigger: { trigger: '.ud-hero', start: 'top top', end: 'bottom top', scrub: 0.6 }
          });

          // reveal elements using ScrollTrigger
          gsapLib.utils.toArray('.reveal').forEach(elem => {
            gsapLib.fromTo(elem, { opacity: 0, y: 14 }, {
          ...
          gsapLib.utils.toArray('.reveal-clip').forEach(elem => {
            gsapLib.fromTo(elem, { clipPath: 'inset(0 0 100% 0)', opacity: 0 }, {
          ...
          if (cards.length) {
            gsapLib.from(cards, { opacity: 0, y: 22, stagger: 0.12, duration: 0.9, ease: 'power2.out', scrollTrigger: { trigger: cards[0], start: 'top 88%' } });
          }

          // subtle hover micro-interaction for post cards
          gsapLib.utils.toArray('.post-card').forEach(card => {
            card.addEventListener('mouseenter', () => gsapLib.to(card, { scale: 1.02, duration: 0.22 }));
            card.addEventListener('mouseleave', () => gsapLib.to(card, { scale: 1, duration: 0.22 }));
          });
          ...
          if (newsletter) {
            newsletter.addEventListener('submit', e => {
              ...
              gsapLib.to(btn, { scale: 0.96, duration: 0.08 }).to(btn, { scale: 1, duration: 0.16 });
              gsapLib.to('.newsletter-card', { boxShadow: '0 0 0 6px rgba(95,132,255,0.06)', duration: 0.18 }).to('.newsletter-card', { boxShadow: '8px 12px 30px rgba(16,40,80,0.04)', duration: 0.22 });
              // simple success toast
              const el = document.createElement('div'); el.className='toast'; el.textContent='Subscribed — check your inbox'; document.body.appendChild(el);
              gsapLib.fromTo(el, { y: 8, opacity: 0 }, { y: 0, opacity: 1, duration: 0.28, onComplete(){ setTimeout(()=>{ gsapLib.to(el,{opacity:0,duration:0.36,onComplete:()=>el.remove() })},1500) } });
            });
          }
          ...
          if (featureCards.length) {
            featureCards.forEach((card, i) => {
              ...
              card.addEventListener('mouseenter', () => gsapLib.to(card, { y: -6, scale: 1.01, duration: 0.18, boxShadow: '10px 14px 30px rgba(16,40,80,0.06)' }));
              card.addEventListener('mouseleave', () => gsapLib.to(card, { y: 0, scale: 1, duration: 0.18, boxShadow: 'none' }));
            });
          }
          ...
          if (heroVisual && window.innerWidth > 720) {
            ...
            heroVisual.addEventListener('mousemove', e => {
              ...
              gsapLib.to('.ud-hero-visual img', { x: dx * 8, y: dy * 6, rotation: dx * 0.8, duration: 0.5, ease: 'power3.out' });
            });
            heroVisual.addEventListener('mouseleave', () => gsapLib.to('.ud-hero-visual img', { x: 0, y: 0, rotation: 0, duration: 0.6, ease: 'power3.out' }));
          }

          // micro-interactions for nav links
          document.querySelectorAll('.nav-links a').forEach(link => {
            link.addEventListener('mouseenter', () => gsapLib.to(link, { y: -3, duration: 0.12 }));
            link.addEventListener('mouseleave', () => gsapLib.to(link, { y: 0, duration: 0.12 }));
          });
        } else {
  ...
  window.postBlog = function() {
    ...
    // Clear inputs
    if (document.getElementById("blogText")) document.getElementById("blogText").value = "";
